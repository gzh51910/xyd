...
export default {
  ...
  methods: {
    ...
    tMove(e) {
      if (this.flag) {
        ...
        if (
          (this.$end - this.$start > 0 && this.index !== 0) ||
          (this.$end - this.$start < 0 && this.index !== this.$length - 1)
        ) {
          // 限止在第一张的时候不能向右拖，在最后一张的时候不能向左拖
          this.$refs["ul"].style.marginLeft =
            this.eachLeft[this.index] + this.$end - this.$start + "px";
        }
      }
    },
    tEnd(e) {
  ...
  mounted() {
    ...
    this.$refs["banner"].children[0].children[0].style.width =
      this.$width + "px";
    let that = this; //that赋值,等同于this

    //获取DOM元素节点-[banner]-[span]+s
    let spans = document
      .getElementById("bannerTag")
    ...
    for (let i = 0; i < spans.length; i++) {
      ...
      spans[i].addEventListener(this.goTouchStart, function(event) {
        ...
        if (that.index !== this.idx) {
          ...
          that.goAnimate(
            parseInt(that.$refs["ul"].style.marginLeft),
            that.eachLeft[that.index]
          );
        }
      });
    }
