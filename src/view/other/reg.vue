<template>
  <div class="container bgF">
    ...
    <ul class="formCom form-login">
      ...
      <li :class="showPwdTwo?'icon-eye1':'icon-eye2'">
        <label>
          <span>确认密码</span>
          <input
            :type="showPwdTwo?'password':'text'"
            placeholder="请输入6-12位确认密码"
            v-model.trim="pwdTwo"
          />
    ...
    <div class="agreeMent mt20" :class="{ no : !checked }" @click="checked = !checked">
      我已阅读并同意
      <span class="blue" @click.stop="goAgreem('/agreement')">《草根信贷用户协议》</span>
  ...
  methods: {
    goReg(phone, pwdTwo, pwdOne) {
      ...
      if (this.checked === false) this.$dialog("请阅读并同意协议");
        ...
        this.imgCode.toUpperCase() !== this.canvasCode.codeNums.toUpperCase()
      )
        this.$dialog("图片验证码不正确");
      else if (checkPwdOne.test(this.pwdOne) === false)
        this.$dialog("密码格式不正确");
